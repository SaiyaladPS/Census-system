...
$ty_id = $_GET['select_id'];
$sql_select = "SELECT * FROM type WHERE ty_id = '" . $ty_id . "'";
...
<body id="page-top">
    ...
    <div class="my-auto">
        <form class=" w-50 h-50 bg-gradient-success text-white px-5 py-5 coutnform rounded-4 mt-4" id="form_province">
            <div class="mb-3">
                ...
                <label for="exampleInputEmail1" class="form-label">ປະເພດສຳມະໂນ</label>
                <input type="text" class="form-control" id="type_name" value="<?php echo $rows_select['ty_name'] ?>"
                    aria-describedby="emailHelp">
            </div>
            <div class="mb-3">
                <label for="exampleInputPassword1" class="form-label">ໜາຍເຫດ</label>
                <input type="text" class="form-control" value="<?php echo $rows_select['ty_remak'] ?>" id="type_remak">
            </div>
    ...
    $(document).ready(() => {
        $('#form_province').submit((e) => {
            ...
            var type_remak = $("#type_remak").val()
            var ty_id = $("#ty_id").val()

            if (type_name == "") {
                Swal.fire({
                    icon: "error",
                    title: "ກະລຸນາປ້ອນປະເພດສຳມະໂນຄົວ",
